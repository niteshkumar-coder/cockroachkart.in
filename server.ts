...
app.get(["/auth/callback", "/auth/callback/"], async (req, res) => {
  ...
  try {
    // Exchange Google auth validation token
    ...
    // Securely bridge login success coordinates back to SPA
    return res.send(`
      <html>
...
// Configure Vite dynamic middleware integration for dev
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`🚀 Fully Functional Server is running at http://0.0.0.0:${PORT}`);
  });
}
